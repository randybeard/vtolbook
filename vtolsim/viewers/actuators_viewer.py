...
class actuatorsViewer:
    def __init__(self):
        ...
        self.plotter = Plotter(plotting_frequency=100, # refresh plot every 100 time steps
                               ...
                               window_title = "Actuators") # name the window
        ...
        # motor_rear_angle = PlotboxArgs(plots=['servo_rear'],
                               ...
                               # time_window=time_window_length)
        motor_right_angle = PlotboxArgs(plots=['servo_right', 'servo_right_c'],
                               rad2deg=True,
                               labels={'left': 'angle (deg)', 'bottom': 'Time (s)'},
                               time_window=time_window_length)
        motor_left_angle = PlotboxArgs(plots=['servo_left', 'servo_left_c'],
                               rad2deg=True,
                               labels={'left': 'angle (deg)', 'bottom': 'Time (s)'},
                               time_window=time_window_length)
        ...
        self.plotter.define_input_vector('controls', 
                ['throttle_rear', 'throttle_right', 'throttle_left', 
                    'servo_right_c', 'servo_left_c',
                    'elevon_right', 'elevon_left'])
        self.plotter.define_input_vector('servo_actual', ['servo_right', 'servo_left'])

        # plot timer
        self.time = 0.

    def update(self, controls, servo_actual, ts):
        ...
        servo_actual = np.squeeze(servo_actual)

        self.plotter.add_vector_measurement('controls', controls_list, self.time)
        self.plotter.add_vector_measurement('servo_actual', servo_actual, self.time)
