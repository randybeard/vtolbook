...
def compute_tf_model(vtol, trim_state, trim_input):
    # trim values
    vtol._state = trim_state
    vtol._update_velocity_data()
    Va_trim = vtol._Va
    alpha_trim = vtol._alpha
    phi, theta_trim, psi = Quaternion2Euler(trim_state[6:10])

    # define transfer function constants
    a_phi1 = -0.5 * VTOL.rho * Va_trim**2 * VTOL.S_wing * VTOL.b * VTOL.C_p_p * VTOL.b / 2.0 / Va_trim
    a_phi2 = 0.5 * VTOL.rho * Va_trim**2 * VTOL.S_wing * VTOL.b * VTOL.C_p_delta_a
    a_theta1 = -VTOL.rho * Va_trim**2 * VTOL.c * VTOL.S_wing / 2.0 / VTOL.Jy * VTOL.C_m_q * VTOL.c / 2.0 / Va_trim
    a_theta2 = -VTOL.rho * Va_trim**2 * VTOL.c * VTOL.S_wing / 2.0 / VTOL.Jy * VTOL.C_m_alpha
    a_theta3 = VTOL.rho * Va_trim**2 * VTOL.c * VTOL.S_wing / 2.0 / VTOL.Jy * VTOL.C_m_delta_e
    a_beta1 = -(VTOL.rho * Va_trim * VTOL.S_wing) / 2.0 / VTOL.mass * VTOL.C_Y_beta
    a_beta2 = (VTOL.rho * Va_trim**2 * VTOL.S_wing) / 2.0 / VTOL.mass * VTOL.C_Y_delta_r

    #feedforwardgain = a_theta2 / a_theta3

    # Compute transfer function coefficients using new propulsion model
    delta_e_trim = trim_input.item(0) + trim_input.item(1)
    delta_t_trim = 0.5*(trim_input.item(3) + trim_input.item(4))
    a_V1 = VTOL.rho * Va_trim * VTOL.S_wing / VTOL.mass * (
            VTOL.C_D_0
            + VTOL.C_D_alpha * alpha_trim
            + VTOL.C_D_delta_e * delta_e_trim
            ) - dT_dVa(vtol, Va_trim, delta_t_trim)/VTOL.mass
    a_V2 = dT_ddelta_t(vtol, Va_trim, delta_t_trim) / VTOL.mass
    a_V3 = VTOL.gravity * np.cos(theta_trim - alpha_trim)

# write transfer function gains to file
    file = open('transfer_function_coef.py', 'w')
    file.write('import numpy as np\n')
    file.write('x_trim = np.array([[%f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f]]).T\n' %
               (trim_state.item(0), trim_state.item(1), trim_state.item(2), trim_state.item(3),
                ...
                trim_state.item(12), trim_state.item(13), trim_state.item(14)))
    file.write('u_trim = np.array([[%f, %f, %f, %f, %f, %f, %f]]).T\n' %
               (trim_input.item(0), trim_input.item(1), trim_input.item(2), trim_input.item(3),
                trim_input.item(4), trim_input.item(5), trim_input.item(6)))
...
# define transfer functions
    ...
    T_chi_phi = transferFunction(np.array([[VTOL.gravity / Va_trim]]), np.array([[1, 0]]), Ts)
...
def dT_dVa(vtol, Va, delta_t):
    # returns the derivative of motor thrust with respect to Va
    eps = 0.01
    T_eps, Q_eps = vtol._motor_thrust_torque(Va+eps, delta_t, False)
    T, Q = vtol._motor_thrust_torque(Va, delta_t, False)
    return (T_eps - T) / eps

def dT_ddelta_t(vtol, Va, delta_t):
    # returns the derivative of motor thrust with respect to delta_t
    eps = 0.01
    T_eps, Q_eps = vtol._motor_thrust_torque(Va, delta_t+eps, False)
    T, Q = vtol._motor_thrust_torque(Va, delta_t, False)
    return (T_eps - T) / eps
