...
Va = 15
Tmax = 40
A = -mass*gravity*np.sin(gam) + np.sqrt(Tmax**2 - mass**2 * gravity**2 * np.cos(gam)**2)
t_hat = Va / A
pos0 = (Va**2 / 2 / A) *np.array([[np.cos(gam)], [0.], [-np.sin(gam)]])  # initial position in inertial frame
vel0 = Va * np.array([[-np.cos(gam)], [0.], [np.sin(gam)]])  # initial velocity in inertial frame
